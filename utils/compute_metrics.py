...
dict_path = './utils/words'  # downloaded from https://gist.githubusercontent.com/wchargin/8927565/raw/d9783627c731268fb2935a731a618aa8e95cf465/words
...
def compute_metrics(completions: Iterable[str], lang: str) -> dict[str, float]:
    ...
    for idx, completion in enumerate(completions):
        ...
        if lines:
            ...
            if line_errors == len(lines):
                full_error_ids.append(idx)
            elif line_errors == 0:
                acc_ids.append(idx)
            else:  # partial error
                partial_error_ids.append(idx)

    metrics = {}
    metrics['acc'] = (sum(line_acc) / len(line_acc) if line_acc else 1.0)*100
    metrics['lpr'] = (1 - with_line_errors / max(1, non_skipped))*100
    if lang in ('ar', 'hi', 'ja', 'ko', 'ru', 'zh'):  # WPR is inaccurate for latin-script languages
        metrics['wpr'] = (1 - with_word_errors / max(1, non_skipped - with_line_errors))*100
    error_ids = {
        'full': full_error_ids,
        'partial': partial_error_ids,
        'acc': acc_ids
    }
    return metrics, error_ids
...
def compute_all_metrics(outputs: list[dict]) -> dict[str, dict[str, float]]:
    ...
    for (source, lang), outputs in itertools.groupby(outputs, key=group_key):
        ...
        metrics, error_ids = compute_metrics(completions, lang)
        all_metrics[(source, lang)] = metrics
        all_error_ids[(source, lang)] = error_ids
    ...
    return all_metrics, all_error_ids
...
if __name__ == '__main__':
    ...
    # Rename column "new_output" to "completion" if it exists
    with open(args.csv_file) as csv_file:
        reader = csv.DictReader(csv_file)
        if 'new_output' in reader.fieldnames:
            outputs = [{**row, 'completion': row.pop('new_output')} for row in reader]
        elif 'generated_text' in reader.fieldnames:
            # outputs = [{**row, 'completion': row.pop('generated_text'), \
                # 'model': 'meta-llama/Meta-Llama-3-8B-Intervene'} for row in reader]
            outputs = [{**row, 'completion': row.pop('generated_text'), \
                'model': 'Mistral-3-7B-Intervene'} for row in reader]
        else:
            outputs = list(reader)
        # if 'model' not in reader.fieldnames:
        #     outputs = [{**row, 'model': 'meta-llama/Meta-Llama-3-8B-Intervene'} for row in reader]
    # with open(args.csv_file) as csv_file:
    #     reader = csv.DictReader(csv_file)
    #     outputs = list(reader)
    ...
    output_name = args.csv_file.split('/')[-1]
    output_csv_file = f"{args.output_path}/{os.path.splitext(output_name)[0]}_results.csv"
    error_output_file = f"{args.output_path}/{os.path.splitext(output_name)[0]}_errors.csv"
    with open(output_csv_file, mode='w', newline='') as csvfile:
        with open(error_output_file, mode='w', newline='') as error_csvfile:
            error_writer = csv.writer(error_csvfile)
            error_writer.writerow(['task', 'model', 'source', 'language', 'acc_ids', 'full_error_ids', 'partial_error_ids'])
            
            csv_writer = csv.writer(csvfile)
            csv_writer.writerow(['task', 'model', 'source', 'language', 'lpr', 'wpr', 'acc'])

            for (task, model), outputs_ in itertools.groupby(outputs, key=group_key):
                all_metrics, all_error_ids = compute_all_metrics(outputs_)

                for (source, lang), metrics in all_metrics.items():
                    lpr = f"{round(metrics['lpr'],2)}"
                    wpr = f"{round(metrics['wpr'],2)}" if 'wpr' in metrics else 'N/A'
                    acc = f"{round(metrics['acc'],2)}"
                    csv_writer.writerow([task, model, source, lang, lpr, wpr, acc])
                    
                for (source, lang), error_ids in all_error_ids.items():
                    error_writer.writerow([task, model, source, lang, error_ids['acc'], error_ids['full'], error_ids['partial']])
                    
    print(f"Errors written to {error_output_file}") 
    print(f"Metrics written to {output_csv_file}")